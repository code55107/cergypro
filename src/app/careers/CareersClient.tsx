            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filtered.map((job, i) => (
                <AnimateIn key={job.id} animation="fadeUp" delay={i % 2 * 120}>
                  <Link href={`/careers/${job.id}`} className="group block bg-white border border-gray-200 p-6 rounded-sm hover:border-amber-400/30 transition-all card-lift">
                    <div className="flex items-start justify-between mb-3">
                      <h3 className="text-lg font-semibold text-gray-900 group-hover:text-amber-500 transition-colors">{job.title}</h3>
                      <span className="text-gray-400 group-hover:text-amber-500 transition-colors shrink-0 ml-4" aria-hidden="true">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="btn-shine px-5 py-2 bg-gray-900 text-white text-xs font-semibold tracking-wider uppercase group-hover:bg-gray-800 transition-colors">VIEW & APPLY</span>
                    </div>
                  </Link>
                </AnimateIn>
              ))}
